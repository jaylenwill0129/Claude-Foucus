...
// gateway with a zero-setup HTTP channel (Resend email) that actually works
// here. Hermes/agents push briefs, approval requests, and alerts to the signed-in
// operator's own email. Self-notification to the account owner — still triggered
// by the operator in-app, never an autonomous external send.
//
...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET") {
    return json({ connector: "operator_relay", configured: Boolean(apiKey && fromEmail), channel: "email" });
  }
  if (req.method !== "POST") return json({ error: "POST required" }, 405);
  if (!apiKey || !fromEmail) return json({ error: "RESEND_API_KEY and RESEND_FROM_EMAIL are required" }, 503);
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: fromEmail, to: [userData.user.email], subject: title, html }),
    });
